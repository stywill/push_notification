...
<html>
    ...
    <body>
        <h2>Simples mensagem menu</h2>
        <a href="index.php">Home</a> |	<a href="logout.php">Sair</a>
        <hr>
        ...
        if (isset($_POST['submit'])) {
            if (isset($_POST['msg']) and isset($_POST['time']) and isset($_POST['loops']) and isset($_POST['loop_every']) and isset($_POST['user'])) {
                ...
                $user = $_POST['user']; /* save Notification */ $save = $sql->saveNotif($msg, $time, $loop, $loop_every, $user);
                if ($save[0] == true) {
                    echo '* notificação salva com sucesso';
                } else {
                    echo 'erro ao salvar : ' . $save[1];
                }
            } else {
                echo '* preencha os campo';
            }
        }
        ?>
        <table border=1>
